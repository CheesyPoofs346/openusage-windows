...
public struct UsageReader {
    ...
    /// Which local-API wire shape the one-shot read emits.
    public enum Wire: Sendable {
        /// The stable, normalized `/v1/limits` contract (default; what the macOS CLI ships).
        case limits
        /// The richer legacy `/v1/usage` payload — every metric line per provider (meters, spend, and
        /// text rows). The Windows system-tray UI uses this to render the full popover.
        case usage
    }

    public func read(providerID requestedProviderID: String? = nil, force: Bool = false, wire: Wire = .limits) async throws -> UsageReadResult {
        ...
        let base = wire == .usage ? "usage" : "limits"
        let path = requestedToken.map { "/v1/\(base)/\($0)" } ?? "/v1/\(base)"
        ...
        // The `/v1/usage` array only carries providers that produced a snapshot, so a credentialed-but-
        // failing provider (e.g. Codex whose token was invalidated) would silently vanish. For the UI
        // wire, wrap the array with the per-provider errors so the tray can show "sign in again" instead
        // of hiding the provider entirely. Display names come from the registry (title-cased fallback).
        if wire == .usage {
            let nameByID = Dictionary(uniqueKeysWithValues: registry.providers.map { ($0.id, $0.displayName) })
            let errorObjects = orderedIDs.compactMap { id -> [String: String]? in
                guard let message = errors[id] else { return nil }
                return ["providerId": id, "displayName": nameByID[id] ?? id.capitalized, "message": message]
            }
            let errorsData = (try? JSONSerialization.data(withJSONObject: errorObjects)) ?? Data("[]".utf8)
            var wrapped = Data(#"{"providers":"#.utf8)
            wrapped.append(data)
            wrapped.append(Data(#","errors":"#.utf8))
            wrapped.append(errorsData)
            wrapped.append(Data("}".utf8))
            return UsageReadResult(data: wrapped, warnings: warnings)
        }
