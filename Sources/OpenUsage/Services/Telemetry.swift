...
enum TelemetryConfig {
    ...
    // Redacted in the Windows fork (telemetry is inert here anyway — no PostHog SDK on Windows).
    // Set OPENUSAGE_POSTHOG_TOKEN at runtime, or restore upstream's token, to enable analytics.
    private static let bakedToken = "phc_REPLACE_ME"
...
final class PostHogTelemetrySink: TelemetrySink {
    ...
    init(enabled: Bool, token: String = TelemetryConfig.token, host: String = TelemetryConfig.host) {
        ...
        #if !canImport(PostHog)
        // No PostHog SDK on this platform (the Windows CLI build); the sink stays inert — no setup,
        // no network — while the rest of the telemetry seam still compiles and the toggle still works.
        configured = false
        AppLog.info(.config, "telemetry inert: analytics SDK unavailable on this platform")
        return
        #else
        configured = true
