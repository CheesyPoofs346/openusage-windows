...
struct ClaudeDesktopSafeStorageKeyReader: ClaudeDesktopSafeStorageKeyReading {
    ...
    func readPassword(allowInteraction: Bool) throws -> String? {
        #if !canImport(Security)
        // The Claude Desktop safe-storage key lives in the macOS Keychain; no equivalent source on
        // this platform, so this credential path is simply unavailable (callers fall back to others).
        return nil
        #else
...
struct ClaudeDesktopAuthStore: Sendable {
    ...
    static func deriveKey(password: String) throws -> Data {
        #if !canImport(CommonCrypto)
        // Unreachable on non-Apple platforms: the safe-storage password is Keychain-only, so no key is
        // ever produced to derive from. Present only so the type compiles.
        throw ClaudeDesktopCredentialError.invalidSafeStorageKey
        #else
