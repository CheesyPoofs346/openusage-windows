...
struct LocalTextFileAccessor: TextFileAccessing {
    ...
    func writeText(_ path: String, _ text: String) throws {
        ...
        #if !canImport(Darwin)
        // Windows: the per-user profile directory is already ACL-scoped to the current user, and
        // Foundation's atomic write does temp-file-then-rename. ponytail: no explicit 0600 equivalent
        // (Windows uses ACLs, not POSIX mode bits) — add SetNamedSecurityInfo if a multi-user threat
        // model ever demands per-file lockdown here.
        try Data(text.utf8).write(to: URL(fileURLWithPath: expanded), options: [.atomic])
        return
        #else
...
struct SecurityKeychainAccessor: KeychainAccessing {
    ...
    func genericPasswordExists(service: String) -> Bool? {
        ...
        #else
        // No Keychain on this platform; "unknown" so callers pick their own safe side.
        return nil
        #endif
    }
