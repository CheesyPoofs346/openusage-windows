...
struct AntigravityUsageClient: Sendable {
    ...
    // Redacted in the Windows fork (Antigravity reads the macOS Keychain, which doesn't exist on
    // Windows, so this installed-app OAuth client is unused here). Restore upstream's value to use it.
    static let googleClientSecret = "GOCSPX-REDACTED"
