...
import Darwin
#else
// Windows has no flock(2); the menu-bar app never runs here, so the only writers are CLI processes.
// The in-process NSLock already serializes those, making the cross-process advisory lock a no-op.
private let LOCK_SH: Int32 = 0
private let LOCK_EX: Int32 = 0
private let LOCK_NB: Int32 = 0
#endif
import Foundation

/// `.EWOULDBLOCK` only exists in `POSIXErrorCode` on Apple platforms; the non-blocking lock never
/// blocks on Windows (the lock is a no-op), so this is always false there.
private func posixErrorIsWouldBlock(_ error: POSIXError) -> Bool {
    #if canImport(Darwin)
    return error.code == .EWOULDBLOCK
    #else
    return false
    #endif
}
...
actor JSONLScanCacheWriter {
    ...
    func pruneStaleIdentities(
        ...
        for directory in contents where directory.lastPathComponent.hasPrefix(prefix) {
            ...
            } catch let error as POSIXError where posixErrorIsWouldBlock(error) {
                continue
            } catch {
                AppLog.warn(
                    .cache,
    ...
    private static func withLock<Result>(
        ...
        #if !canImport(Darwin)
        // No advisory file locking on Windows; the in-process NSLock is the only ordering needed here.
        return try body()
        #else
        try FileManager.default.setAttributes(
